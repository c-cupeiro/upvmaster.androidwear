...
/**
 * Created by ccupeiro on 29/06/2017.
 */
...
public class PaginatedCursor extends AbstractCursor {
            ...
            if (!mCachedRows[row]) {
                for (int col = 0; col < mColumnCount; col++) {
                    switch (mCursor.getType(col)) {
                        case Cursor.FIELD_TYPE_BLOB:
                            mByteArrayDataCache[row][mByteArrayCacheIndexMap[col]] = mCursor.getBlob(col);
                            break;
                        case Cursor.FIELD_TYPE_FLOAT:
    ...
    public boolean onMove(int oldPosition, int newPosition) {
        if ((newPosition - oldPosition) != 1 || (newPosition + PAGE_THRESHOLD) <= mLastCachePosition) {
